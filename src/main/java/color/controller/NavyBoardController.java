...
public class NavyBoardController {
    ...
    @PostMapping()
    public Long create(@Valid @AuthenticationPrincipal CustomUserDetails userDetails, @RequestBody NavyBoardCreateDTO boardCreateDTO) {
        return boardService.create(userDetails.getId(), boardCreateDTO.getTitle(), boardCreateDTO.getContent());
    }

    @PutMapping("/edit/{id}")
    public Long update(@Valid @AuthenticationPrincipal CustomUserDetails userDetails, @PathVariable Long id, @RequestBody NavyBoardUpdateDTO boardUpdateDTO) throws AccessDeniedException {
